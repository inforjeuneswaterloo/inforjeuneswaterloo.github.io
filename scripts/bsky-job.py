...
# Ces variables doivent être définies comme Secrets dans votre workflow GitHub
...
DATA_DIR = "_data"
TARGET_TAG = "bwaterloo" # <--- TAG CIBLE
OUTPUT_FILE = os.path.join(DATA_DIR, "bluesky_job_waterloo_posts.json") # <--- NOM DU FICHIER
...
def fetch_bluesky_feed():
    """Authentifie, récupère le flux, filtre par tag/date et sauvegarde."""
    ...
    try:
        ...
        for item in data.get('feed', []):
            ...
            if post:
                ...
                # 3a. Nettoyage du texte (suppression des URLs)
                post_text = post['record']['text']
                cleaned_text = re.sub(URL_REGEX, '', post_text, flags=re.IGNORECASE).strip()
                post['record']['text'] = cleaned_text 

                # 3b. Extraction du titre (première ligne)
                post['title'] = cleaned_text.split('\n', 1)[0].strip()

                # 3c. Suppression de l'embed d'article externe (pour ne pas garder titre/vignette)
                ...
                # 3d. Ajout de l'URL de l'image simplifiée (si c'est une image directe)
                post['image_url'] = get_post_image_url(post)
                
                # 3e. Ajout à la liste finale 
                filtered_posts.append(post)

        # 4. Sauvegarde finale du flux filtré (régénération du fichier)
        save_data_to_json(filtered_posts, OUTPUT_FILE)
