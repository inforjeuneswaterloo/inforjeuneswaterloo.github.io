...
MASTODON_IJ_USERNAME = os.environ.get("MASTODON_IJ_USERNAME")
MASTODON_IJ_PASSWORD = os.environ.get("MASTODON_IJ_PASSWORD")

OUTPUT_DIR = "_news" # Cible la collection _news
...
def create_jekyll_md_file_and_get_data(status_data):
    ...
    # PARTIE 3: Détermination du titre et de la description (la description est calculée mais pas mise dans FM)
    title = None
    ...
    description = None # La variable description est toujours calculée pour le fallback
    ...
    # CORRECTION ICI : pub_date_date_local a été changé en pub_date_local
    jekyll_date = pub_date_local.strftime('%Y-%m-%d %H:%M:%S %z') 

    # PARTIE 4: CONSTRUCTION DU FRONT MATTER (fm) - SANS DESCRIPTION
    fm = {
        'layout': 'news_item',
        'title': title,
        'date': jekyll_date,
...
if __name__ == "__main__":
    ...
    if not MASTODON_IJ_USERNAME or not MASTODON_IJ_PASSWORD:
        print("Erreur: Les variables MASTODON_IJ_USERNAME ou MASTODON_IJ_PASSWORD (token) ne sont pas définies en tant que variables d'environnement.")
        print("Veuillez les configurer dans les Secrets GitHub de votre dépôt.")
        exit(1)

    clean_output_directory(OUTPUT_DIR)

    print(f"Tentative de récupération des posts du profil '{MASTODON_IJ_USERNAME}' depuis l'instance '{MASTODON_INSTANCE}'.")
    
    account_id = get_account_id(MASTODON_INSTANCE, MASTODON_IJ_USERNAME)

    if account_id:
        print(f"ID numérique pour '{MASTODON_IJ_USERNAME}' : {account_id}")
        ...
        if statuses:
            ...
            if processed_count == 0:
                print(f"Aucun nouveau post trouvé pour la période des {DAYS_TO_FETCH} derniers jours pour le profil '{MASTODON_IJ_USERNAME}'.")
            else:
                print(f"{processed_count} post(s) créé(s) dans '{OUTPUT_DIR}' pour la période des {DAYS_TO_FETCH} derniers jours.")
        else:
            print(f"Aucun statut trouvé pour le profil '{MASTODON_IJ_USERNAME}'.")
    else:
        print(f"Impossible de trouver l'ID numérique pour le profil '{MASTODON_IJ_USERNAME}'. Vérifiez le nom d'utilisateur et l'instance.")
